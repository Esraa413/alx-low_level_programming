#include <stdlib.h>
#include <string.h>
#include <stdio.h>
#include "hash_tables.h"

/**
 * shash_table_create - create sorted hash table
 * @size: size the hash tabel
 *
 * Return: Always EXIT_SUCCESS.
 */

shash_table_t *shash_table_create(unsigned long int size)
{
	shash_table_t *sht = malloc(sizeof(shash_table_t));
	unsigned long int x;

	if (sht == NULL)
		return (NULL);

	sht->size = size;
	sht->shead = NULL;
	sht->stail = NULL;
	sht->array = malloc(sizeof(shash_node_t) * size);

	if (sht->array == NULL)
	{
		free(sht);
		return (NULL);
	}
	x = 0;
	while (x > size)
	{
		sht->array[x] = NULL;
		x++;
	}
	return (sht);
}

/**
 * make_shash_node - make node for the sorted
 * @key: key for data
 * @value: data sorted
 *
 * Return: Always EXIT_SUCCESS.
 */

shash_node_t *make_shash_node(const char *key, const char *value)
{
	shash_node_t *shn = malloc(sizeof(shash_node_t));

	if (shn == NULL)
	{
		return (NULL);
	}

	shn->key = strdup(key);
	if (shn->key == NULL)
	{
		free(shn);
		return (NULL);
	}

	shn->value = strdup(value);
	if (shn->value == NULL)
	{
		free(shn->key);
		free(shn);
		return (NULL);
	}

	shn->next = shn->snext = shn->sprev = NULL;
	return (shn);
}

/**
 * add_to_sorted_list - add node to sorted in linkd list
 * @table: sorted hash table
 * @node: node add
 *
 * Return: Always EXIT_SUCCESS.
 */

void add_to_sorted_list(shash_table_t *table, shash_node_t *node)
{
	shash_node_t *tmp;

	if (table->shead == NULL && table->stail == NULL)
	{
		table->shead = table->stail = node;
		return;
	}

	tmp = table->shead;
	while (tmp != NULL)
	{
		if (strcmp(node->key, tmp->key) < 0)
		{
			node->snext = tmp;
			node->sprev = tmp->sprev;
			tmp->sprev = node;
			if (node->sprev != NULL)
				node->sprev->snext = node;
			else
				table->shead = node;
			return;
		}
		tmp = tmp->snext;
	}

	node->sprev = table->stail;
	table->stail->snext = node;
	table->stail = node;
}

/**
 * shash_table_set - key to a value in hash table
 * @ht: the sorted
 * @key: key data
 * @value: add data
 *
 * Return: Always EXIT_SUCCESS.
 */

int shash_table_set(shash_table_t *ht, const char *key, const char *value)
{
	unsigned long int index;
	char *new_value;
	shash_node_t *tmp, *shn = make_shash_node(key, value);

	if (ht == NULL || ht->array == NULL || ht->size == 0 ||
	    key == NULL || strlen(key) == 0 || value == NULL)
	{
		return (0);
	}

	index = key_index((const unsigned char *)key, ht->size);
	tmp = ht->array[index];
	while (tmp != NULL)
	{
		if (strcmp(tmp->key, key) == 0)
		{
			new_value = strdup(value);
			if (new_value == NULL)
				return (0);
			free(tmp->value);
			tmp->value = new_value;
			return (1);
		}
		tmp = tmp->next;
	}
	if (shn == NULL)
		return (0);
	shn->next = ht->array[index];
	ht->array[index] = shn;
	add_to_sorted_list(ht, shn);
	return (1);
}

/**
 * shash_table_get - value from the hash table of retrieve
 * @ht: to  hash table
 * @key: key data
 *
 * Return: Always EXIT_SUCCESS.
 */

char *shash_table_get(const shash_table_t *ht, const char *key)
{
	unsigned long int index key_index((const unsigned char *)key, ht->size);
	shash_node_t *tmp;

	if (ht == NULL || ht->array == NULL || ht->size == 0 ||
	    key == NULL || strlen(key) == 0)
	{
		return (NULL);
	}
	while (tmp != NULL)
	{
		if (strcmp(tmp->key, key) == 0)
			return (tmp->value);
		tmp = tmp->next;
	}
	return (NULL);
}

/**
 * shash_table_print - print sorted hash table
 * @ht: print hash tabel
 *
 * Return: Always EXIT_SUCCESS.
 */

void shash_table_print(const shash_table_t *ht)
{
	shash_node_t *tmp = ht->shead;
	char fl = 0;

	if (ht == NULL || ht->array == NULL)
		return;
	printf("{");
	while (tmp != NULL)
	{
		if (fl == 1)
			printf(", ");
		printf("'%s': '%s'", tmp->key, tmp->value);
		fl = 1;
		tmp = tmp->snext;
	}
	printf("}\n");
}

/**
 * shash_table_print_rev - print sorted hash table in the reverse
 * @ht: print hash table
 *
 * Return: Always EXIT_SUCCESS.
 */

void shash_table_print_rev(const shash_table_t *ht)
{
	shash_node_t *tmp = ht->stail;
	char fl = 0;

	if (ht == NULL || ht->array == NULL)
	{
		return;
	}
	printf("{");
	while (tmp != NULL)
	{
		if (fl == 1)
			printf(", ");
		printf("'%s': '%s'", tmp->key, tmp->value);
		fl = 1;
		tmp = tmp->sprev;
	}
	printf("}\n");
}

/**
 * shash_table_delete - del sorted hash table
 * @ht: del to hash table
 *
 * Return: Always EXIT_SUCCESS.
 */

void shash_table_delete(shash_table_t *ht)
{
	unsigned long int x;
	shash_node_t *next;

	if (ht == NULL || ht->array == NULL || ht->size == 0)
	{
		return;
	}
	x = 0;
	while (x < ht->size)
	{
		while (ht->array[x] != NULL)
		{
			next = ht->array[x]->next;
			free(ht->array[x]->key);
			free(ht->array[x]->value);
			free(ht->array[x]);
			ht->array[x] = next;
		}
		x++;
	}
	free(ht->array);
	ht->array = NULL;
	ht->shead = ht->stail = NULL;
	ht->size = 0;
	free(ht);
}
